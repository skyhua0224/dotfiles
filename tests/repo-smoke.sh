...
assert_grep '^tealdeer$' "$REPO_ROOT/packages/cachyos-pacman.txt"
if grep -Eq '^tlrc$' "$REPO_ROOT/packages/cachyos-aur.txt"; then
  fail "tlrc should not stay in the Arch AUR manifest"
fi
...
assert_grep '^command_timeout = 2000$' "$REPO_ROOT/starship/.config/starship.toml"
assert_grep 'treset' "$REPO_ROOT/README.md"
assert_grep 'DOTFILES_ZOXIDE_CMD=z' "$REPO_ROOT/README.md"
assert_grep 'DOTFILES_AUR_HELPER=yay' "$REPO_ROOT/README.md"
assert_grep 'CLI 配置' "$REPO_ROOT/README.md"
